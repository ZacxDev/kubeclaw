...
Six checks, each closing a different way the guard could pass vacuously:

  SELFTEST    drives every guard below with synthetic inputs and asserts each
              produces ITS OWN error, inline on every run. A one-character
              edit (`in` -> `not in`) otherwise leaves a guard permanently
              inert while the run still prints OK.
...
LEDGERS = {
    ...
    # The api_key auth branch REPLACES the Claude-credentials one.
    "tests/ordering/values-apikey-auth.yaml": [
        m for m in COMMON if m != "Bootstrap agent auth from Claude credentials"
    ] + ["Bootstrap agent auth from API key (*)"],
...
# Floors, so deleting a fixture or a constraint cannot silently shrink coverage
# while the run still prints OK. Raise them alongside real additions; never
...
def evaluate(label, found, expected):
    """Apply every guard to one marker list. Returns (failures, exercised set).

    Pure — no helm, no filesystem — so selftest() can drive it with synthetic
    inputs and prove each guard actually fires.
    """
    ...
    if TERMINAL in found and found[-1] != TERMINAL:
        ...
        failures.append(
            f"{label}: DEAD CODE — {after} render AFTER {TERMINAL!r}, which "
    ...
    for n, (before, after, why) in enumerate(CONSTRAINTS):
        if before in pos and after in pos:
            ...
            if pos[before] > pos[after]:
                failures.append(
                    f"{label}: ORDER VIOLATION — {before!r} (#{pos[before]}) "
                    f"must precede {after!r} (#{pos[after]}): {why}")
    return failures, exercised


def selftest():
    """NEGATIVE CONTROL on this checker's own guards.

    Every guard here is a conditional that a one-character edit turns inert —
    `if TERMINAL in found` -> `not in` leaves the DEAD-CODE check permanently
    silent while the run still prints "'Gateway restart loop' last". Nothing
    watched these guards fail, so their green was unfalsifiable.

    Drives evaluate() with synthetic marker lists and asserts each guard
    produces ITS OWN error. Pure python, so it runs inline on every invocation
    and cannot be skipped.
    """
    clean = ["SSH setup", "Repo clone/pull", TERMINAL]
    cases = [
        ("clean input yields no failures", clean, clean, None),
        ("DEAD CODE", clean[:2] + [TERMINAL, "Trailing"], clean + ["Trailing"],
         "DEAD CODE"),
        ("DUPLICATE", ["SSH setup", "SSH setup", TERMINAL], clean, "DUPLICATE"),
        ("NEW section", clean[:2] + ["Surprise", TERMINAL], clean, "NEW section"),
        ("disappeared", ["SSH setup", TERMINAL], clean, "disappeared"),
        ("ORDER VIOLATION", ["Repo clone/pull", "SSH setup", TERMINAL], clean,
         "ORDER VIOLATION"),
    ]
    for name, found, expected, want in cases:
        failures, _ = evaluate("selftest", found, expected)
        blob = " ".join(failures)
        if want is None:
            if failures:
                raise SystemExit(f"SELFTEST FAILED: {name} produced {failures}")
        elif want not in blob:
            raise SystemExit(
                f"SELFTEST FAILED: {name} did not fire. Expected {want!r}, got "
                f"{failures or 'no failures'}. A guard that cannot fail is not "
                f"a guard.")
    # And the unexercised-constraint guard, which lives outside evaluate().
    if not unexercised({0}):
        raise SystemExit(
            "SELFTEST FAILED: the unexercised-constraint guard did not fire "
            "for a constraint set exercised by nothing.")
    print(f"  selftest: {len(cases)} guards + the unexercised-constraint guard "
          f"all fire")


def unexercised(exercised):
    """Constraints no fixture exercised — a declaration, not a check."""
    out = []
    for n, (b, a, _) in enumerate(CONSTRAINTS):
        if n not in exercised:
            out.append(
...
def main():
    if "--print" in sys.argv:
        # Emits the flat per-file form. LEDGERS is factored as COMMON + [...],
        # so re-fold shared entries by hand rather than pasting these verbatim.
        for vf in LEDGERS:
            print(f'    "{vf}": [')
    ...
    if len(LEDGERS) < MIN_FIXTURES or len(CONSTRAINTS) < MIN_CONSTRAINTS:
        raise SystemExit(
            f"FATAL: coverage floor breached — {len(LEDGERS)} fixtures "
            f"(min {MIN_FIXTURES}), {len(CONSTRAINTS)} constraints "
            f"(min {MIN_CONSTRAINTS}). Deleting either shrinks what this gate "
            f"watches while it still reports OK.")

    failures, exercised, skipped = [], set(), False
    for vf, expected in LEDGERS.items():
        ...
        fs, ex = evaluate(vf, found, expected)
        failures += fs
        exercised |= ex
        if any("DUPLICATE" in f for f in fs):
            skipped = True

    # Suppressed when a fixture was skipped: every constraint would then report
    # "never exercised", burying the real failure under derivative noise.
    if not skipped:
        failures += unexercised(exercised)
    ...
    print(f"\nOK: {len(LEDGERS)} value files, ledgers match, {TERMINAL!r} last, "
          f"{len(CONSTRAINTS)} constraints all exercised")
    return 0
